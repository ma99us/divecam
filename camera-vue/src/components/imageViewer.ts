...
export default class ImageViewer{
    ...
    protected isClosable:      boolean;     // new
    protected isVisible: boolean = false    // new
    ...
    constructor(parameters:constructorParameters){
        ...
        const view  = ImageViewer.getHtml(this.viewID, this.isZoomable = parameters.isZoomable ?? true,
          this.isClosable = parameters.isClosable ?? true);
        let parentEl = parameters.parentId ? document.getElementById(parameters.parentId) : null;
    ...
    //getHtml:
    protected static getHtml(viewID:number, isZoomable:boolean, isClosable:boolean):ChildNode{
        const html = `
    ...
    //scrollToImage:
    protected scrollToImage(index:number, instant: boolean){
        const imagesWrapper = <HTMLElement> this.view.getElementsByClassName('imagesWrapper')[0];
        const imageContainers = imagesWrapper.children;
        const imageContainer = <HTMLElement> imageContainers.item(index);
        ...
        imagesWrapper.scrollTo({left: imageCenterPosition, behavior: instant ? 'instant' : 'smooth'});
    }
    ...
    //setThumbnail:
    protected setThumbnail(index:number, instant: boolean){
        ...
        if(thumbnail !== null){
            ...
            this.scrollThumbnail(index, instant);
        }
    }

    //scrollThumbnail:
    protected scrollThumbnail(index:number, instant: boolean){
        const thumbnailsWrapper = <HTMLElement> this.view.getElementsByClassName('thumbnailsWrapper')[0];
        ...
        thumbnailsWrapper.scrollTo({left: thumbnailCenterPosition, behavior: instant ? 'instant' : 'smooth'});
    }
